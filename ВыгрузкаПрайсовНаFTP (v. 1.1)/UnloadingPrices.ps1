...
$fatClient  = 'C:\Program Files (x86)\1cv8\8.3.11.3034\bin\1cv8.exe'
$1cAdmPass  = 'A12pAR'
$1cAdm      = 'јдминистратор'
...
$schedulerTaskCred = 'GKUNION\reznikmn' #учетка под которой запускаетс€ скрипт
...
$servers = @($xmlParam.—клады»“ипы÷ен.Ѕаза1—.»м€—ервера)
...
for($i=0;$i -le $bases.Count - 1; $i++){

    $pricesFile = "P_" + $firms[$i] + "_" + $clients[$i] + "_" + $pricesTypesNames[$i] + "_" + $warehousesNames[$i]
    $runParam = $pricesTypes[$i] + "**" + $warehouses[$i] + "**" + $pricesFile
    $runParam = '"'+$runParam+'"'

    $enterpriseArg = "/S " + $servers[$i] + "\" + $bases[$i] + " /N $1cAdm /P $1cAdmPass /Debug /C $runParam `
    /Execute $sharePath\¬ыгрузка÷енƒл€—айта.epf /WA-"
...
Get-Process -Name *1cv8*,*excel* -IncludeUserName | Where-Object {$_.UserName -eq $schedulerTaskCred} | Stop-Process